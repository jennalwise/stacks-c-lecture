    ...
    if (isFull(s)) {
        ...
        if (n == NULL) {
            ...
            if (isEmpty(s)) {
                ...
                n->min = minHelper(s->top->value, item);
            }
            n->value = item;
            n->next = s->top;
        }
...
/* If the Stack is not empty, the min item is returned, but the contents of the Stack are not changed.
*  If the Stack is empty, nothing is returned, and an error is reported.
*/
